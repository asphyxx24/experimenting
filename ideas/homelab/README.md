...
Jede `docker-compose.yml` bindet sich per `networks: {default: {name: homelab_net, external: true}}` daran an, statt ein eigenes isoliertes Netzwerk pro Compose-Datei aufzumachen. Grund: Container müssen sich fast durchweg gegenseitig erreichen (eigene Automatisierungs-Skripte ↔ Mosquitto per MQTT, Uptime Kuma überwacht alle anderen Container per Name statt IP) — ein einziges gemeinsames Netzwerk ist hier einfacher als feingranulare Segmentierung pro Dienst-Paar.
...
10. **Mosquitto MQTT** — Broker für ESP32-Projekte (Companion-Watch)
11. **n8n** — Workflow-Automatisierung
12. **Uptime Kuma** — Monitore für alle bisherigen Dienste anlegen
13. **Tailscale** — Remote-Zugriff einrichten, danach HTTPS via Caddy für Nextcloud
14. **Cloudflare Tunnel + Access** — nur für Jellyfin-Freigabe an Freunde/Familie
15. **Backup einrichten** — `rsync`-Hardlink-Versionierung auf USB-Stick, Restore-Test
16. **Emulation** — PCSX2 + RetroArch nativ installieren
...
| USB-Stick (256 GB) | Backup (Nextcloud-Daten+DB 50–100 GB, n8n/AdGuard-Configs, Snapshot-Historie) + wichtige Dateien — 128 GB reicht mit der jetzt hinzugekommenen Versionierung zu knapp | ~20–30 € |
...
**Priorität:** Zigbee-Dongle + Energiesteckdose zuerst (zusammen unter 50€, direkt nach der Linux-Installation nutzbar). USV ist bewusst zurückgestellt (siehe Abschnitt "Optional / Zurückgestellt") — kein Dauerbetrieb-Grund aktuell.
